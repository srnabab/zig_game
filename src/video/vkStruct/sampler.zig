...
pub fn _createSampler(device: vk.VkDevice, pAllocCallBacks: [*c]vk.VkAllocationCallbacks, allocator: std.mem.Allocator, ID: u32, anisotropy: f32) !vk.VkSampler {
    var info = try samplerRead.readSampler(ID, allocator);
...
pub fn initSamplers(self: *Self, device: vk.VkDevice, pAllocCallBacks: [*c]vk.VkAllocationCallbacks, allocator: std.mem.Allocator) !void {
    self.pixel2dSampler = try _createSampler(device, pAllocCallBacks, allocator, comptime file.comptimeGetID("pixel2dSampler.sampler"), 1.0);
}
