...
- **名字注册表**：`str_maps: pass_buffer_map` 是 comptime 常量，构建时先造 `var ctx = CTX{ .buffers = &.{}, .passes = &.{} }`，再 `setPass.setting(&ctx)` 把全部 pass/buffer 名填入 ctx，然后 `deduplicateStrings` + `StaticStringMap` 生成 `passes` / `buffers` 两张表；`belongMap` 记录每个名字属于哪张表；`files` 直接复用 `file.fileNameID.FileNameIdHashMap`。
- `CTX = struct { passes: []const []const u8, buffers: []const []const u8 }`——这是 renderFlow/setPass 在 comptime 登记名字用的上下文。
- API：
  ...
  - `toStr2(str: Str2) Str`——运行时版；debug 下查 `ID2` 失败会 log 并返回 `maxInt(u32)`。
...
- build.zig：给涉事模块(`u8pack_mod/renderFlow_mod/pass_mod/video_mod/resource_mod/setPass_mod/loadmap_mod/instances2/resourceProcess_mod` 等)补 `addImport("u8pack")`。
...
- `.watching`（watcher 启动目录下）逐行列出要监听的目录；当前内容：
  `Assets\Shaders`、`Assets\Sampler`、`Assets\pipeline`、`Assets\loadMap`、`Assets\instance`、`zig-out\bin\Content`、`tools\srcs\cooker`。改动 `.watching` 本身会热更新监听集合。
- watcher 把文件事件按 `<action>?<name>?<fullPath>?<type>\n` 协议写给 cooker；`action>1000` 表示"该文件在 Content/ 内"。cooker 侧按 `ProcessType` 分派到 `src/resourceProcess/*.zig` 里对应的 `*_Cooker`（`Enable` 为 true 才执行）：
  ...
  - `.instance` → `Instance_Cooker.preProcess2`：用 `pwsh Copy-Item` 复制到 `Content/Instance/`。
...
- **Custom structs + src/build pipeline**: `vertices`/`meshInstance`/`mesh`/`passGroupMapping` 四个自定义结构体已从 `monsoon/video/`(及 indirect2D/) 拆到 `src/customStruct/*.zig`。模块名与文件名一致：`src/customStruct/meshInstance.zig`(原名 instance.zig)，模块名 `"meshInstance"`；`src/customStruct/passGroupMapping.zig`(原名 `monsoon/video/pass/pass.zig`，模块名不变)。它们的 module 创建与接线全部收敛到 `src/build/`：`config.zig` 定义 `Config`(上游 = 结构体源码 @import 的引擎模块 video/processRender/vertexStruct/global/handle/u8pack；下游 = 引擎中回填注册的消费者 exe/resource/resourceProcess/instances2)；每结构体一个构建文件(`vertices.zig`/`meshInstance.zig`/`mesh.zig`/`passGroupMapping.zig`，导出 `name` + `build(cfg)`，内部完成 createModule + 上游 addImport + 对下游消费者 addImport)；`out.zig` 导出 `build(cfg)`，里面**逐个显式**调用各 build 函数（`list`/`Modules` 仍在文件里但当前未被 build() 使用——属预留的自动遍历结构）。根 `build.zig` 在 instance_mod2 创建后构造 `Config` 并 `_ = customBuild.build(cfg)`。新增自定义结构体：加文件 + 在 out.zig 里加一行 `build(cfg)` 调用（list 可顺手加，但不影响构建）。
- **每帧上传管线**: `src/renderUpload.zig` 的 `upload(io, vulkan, passes, pTextureSet, uctx, commands: *processRender.commands)` 是 render 线程每帧入口（render.zig 在 `waitEndFence/startCommand/addCachedCommand` 前调用），依次 `uctx.vertices.uploadInstance(io, vulkan, commands)`、`uctx.instances1.upload(io, vulkan, commands)`、`uctx.passGroupMapping.upload(io, vulkan, commands)`、`uctx.meshes.upload(commands)`，均通过 `commands.cacheCommand(.copyBuffer)` 录命令。
  ...
  - `monsoon/instances/instances.zig` 的 `load(io, passes, pTextureSet, vertices, instances, passGroupMapping, handles)` 在 **render 线程**（render.zig）逐 item 处理：`indirect2D` → `vertices.addInstance` 并写 `pass.userdata.totalSpriteCount`；`i_feather` → `instances1.add` + `passGroupMapping.add` + 写 `pass.userdata`(drawCount) + `setPushConstants(2, toBytes(tidx), 64)`；末尾 free 各 item 的 `textures` 数组并 `clearRetainingCapacity`。
...
- **Resource readers & cookers**: `src/resourceProcess/*.zig` 每个文件同时提供(或只提供其一)：
  - **`*_Cooker`**(编译期/工具期，供 `tools/srcs/cooker` 调用)：`pub const TableName`、`pub const Enable`、`preProcess(io, gpa, dir, parmas: *PreProcessParm, Table: *TableType)`、`preProcess2(io, gpa, contentFolderPath, fileName, content, fullPath, database: *db)`、`judgeFileType2(content, fType) ProcessType`。有 cooker 的：PNG/GLTF/VTX/Sampler/Shader/Pipeline/LoadMap/Instance(在 `src/resourceProcess.zig` 中以 `pub const X_Cooker = file.X_Cooker` 重导出)。
  - **`*_Reader`**(运行时)：`read`(旧名 `processResource`) + 可选 `updateLoad`/`renderLoad`。当前 readers：`KTX2_Reader`/`VTX_Reader`/`PNG_Reader`/`LMap_Reader`/`Binary_Reader`/`Instance_Reader`(均在 `src/resourceProcess.zig` 重导出)。`Shader`/`Pipeline`/`Sampler`/`GLTF`/`LoadMap`/`DIR`/... 等没有 reader，走 `Example_Reader`(由 `TypeUseExample` + `useExample(comptime fType)` 判定；`Example_Reader.read` 内 `unreachable`)。
  ...
  - **`Ctx` 契约**：每个 reader 定义 `pub const Ctx = struct { <字段名>: *<UserContext 里同名字段的类型>, ... }`。消费端(render/update 线程)在拿到队列项后，用 `@typeInfo(field.Ctx)` 遍历字段名，把 `&@field(args.uctx, f.name)` 逐个填进临时 `Ctx` 再传给 load 函数——即 `Ctx` 字段名必须与 `UserContext` 字段名严格一致。例：PNG/KTX2 `{ pTextureSet }`、VTX `{ meshes }`、LMap `{ loadmaps }`、Instance `{ instances2, pTextureSet }`。
  ...
  - `src/resourceProcess.zig` 还导出：`ProcessType`(含 `.LoadMap/.LMap/.Binary/.Instance` 等)、`list`(扩展名→ProcessType)、`Mappings`(ProcessType→`Handles.ResourceType`，用于 `readResource` 建 handle)、`CustomTables`/`CustomTablePack`(ContentPath/ImageLoadParameter/ModelLoadParameter)、`Triggers`(建库触发器 SQL)、`judgeFileTypeByContent`、`preProcessInit(io, gpa, content)`(cooker 启动调：`sceneJsonInit`(读/建 `Content/Scenes.json` → `SceneJson: ?[]cgltf.Scene` + `SceneNameStringMap`/`SceneNodeNames`) + `gltf.saveSceneJson`，`SceneFileName = "Scenes.json"`)。
...
- **Instance reader**: `src/resourceProcess/instance.zig` `Instance_Reader.read` 解析 `.instance` JSON(`Assets/schema/instance.schema.json`)：根 `{items:[…]}` 含可选 `"$schema"`(无 `ignore_unknown_fields`)；item = `{pass, pos, scale, rotation: f32 arrays, textures: []string, model?: string}`。`Child.instances: []instance2.instance` 由 `read` 分配。逐 item：`pos/scale/rotation` 必须长度 3(否则 `Invalid`)，`pass` 用 `ctx.passes.passMap.get(toStr2(item.pass))`(否则 `Invalid`)，textures 用 `resource.getResourceHandle(file.getID(name))` + `Handles.handleIsValid`(否则 `Unavaliable`)，`model` 同 `getResourceHandle`，并 `ctx.handles.createHandle(Handles.Invalid, .others)`。`updateLoad`(在 update 线程)对每个 ins 调 `uctx.instances2.add(io, ins)`，失败返回 `Handles.Invalid`，成功返回 `Handles.WaitFill`。`Child.instances` 与每个 `ins.textures` 都用 `ctx.gpa` 分配(reader 内)；`monsoon/instances/instances.zig` 的 `load()` 末尾用 `self.instances.allocator.free(item.textures)` 释放并 `clearRetainingCapacity`(两者分配器来源不同，注意一致)。
...
- **Instance schema**: `Assets/schema/instance.schema.json` (JSON Schema draft-2020-12) — 根对象 `{items: [...]}`，item 为 `{pass: string, pos/scale/rotation: number[3], textures: string[], model?: string}`；示例 `Assets/instance/test.instance`。(旧的 `scene.schema.json`/`Assets/scene/` 已废弃，改名 instance。)
