...
float EvaluateG(float q1, float q2, float mu, float sigma) {
    ...
    float z2 = (q2 - mu) * sigma;
    float z1 = (q1 - mu) * sigma;
...
float F_p(float v) {
    return -0.97867503 * pow(v, 3.0) + 2.60238128 * v * v + -1.88083816 * v + 0.77743396;
}
...
vec2 RachisMasking(float radius, vec2 v_proj, vec2 r, float centerLine, float e) {
    float W_proj = 2 * radius / (dot(v_proj, r) + e);
    
    float theta = acos(view_TBN.z);
    float W_quote = sqrt(pow(W_proj / 2, 2.0) + pow(radius * tan(theta), 2)) - W_proj / 2;
    
    float r_quote = (W_proj + W_quote) * dot(v_proj, r) * 0.5;

    float c_quote = centerLine + (W_proj / 2) * r.x;

    return vec2(r_quote, c_quote);
}

vec2 BarbMasking(float radius, vec2 v_proj, vec2 r, float centerLine, float e, float spacing) {
    float W_proj = 2 * radius / (dot(v_proj, r) + e);
    
    float theta = acos(view_TBN.z);
    float W_quote_1 = sqrt(pow(W_proj / 2, 2.0) + pow(radius * tan(theta), 2)) - W_proj / 2;

    float W_quote_2 = spacing / dot(v_proj, r) - W_proj;
...
void main() {
    ...
    vec2 v_proj = view_TBN.xy / sqrt(pow(view_TBN.x, 2) + pow(view_TBN.y, 2));
    ...
    float sigma_t_s = sqrt(sigma_t_pow2 - pow(sigma_st, 2) / sigma_s_pow2);

    vec2 res = RachisMasking(r_r_uv, v_proj, vec2(1, 0), 0.5, 0.00005);
    float r_r_uv_ = res.x;
    float c_r_ = res.y; 
    float w_r = EvaluateG(c_r_.x - r_r_uv_, c_r_.x + r_r_uv_, x, sigma_x);
    ...
    if (far) {
        ...
        const float phase_bd_0 = 1.2;
        const float phase_bp_0 = 1.2;

        const float q_0 = 1.1;
        const float g_0 = 1.1;

        // barb
        vec2 U_b = (2 * PI / p_bl) * normal_b;

        float A_b = dot(U_b, Sigma * U_b);
        int k_g_b_max = int(ceil(sqrt(2 * threshold / A_b)));
        int k_g_b_min = -k_g_b_max;
            
        vec2 masking_b = BarbMasking(r_b_uv, v_proj, normal_b, g_0, 0.00005, p_b - 2 * r_b_uv);
        ...
        for (int i = k_g_b_min;i <= k_g_b_max;i++) {
            ...
            w_b += (2 * masking_b.x / p_b) * sinc(omega_kg_b * masking_b.x) * 
            exp(-0.5 * dot(omega_b, Sigma * omega_b)) * cos(omega_kg_b * masking_b.y - dot(omega_b, mean));
        }
        ...
        int k_q_bd_min = -k_g_bd_max;
        
        vec2 masking_bd = BarbMasking(r_bl_uv, v_proj, n_bd, phase_bd_0, 0.00005, d_bl_uv);
        ...
        for(int g = k_g_bd_min;g <= k_g_bd_max;g++) {
            for(int q = k_q_bd_min;q <= k_q_bd_max;q++) {
                ...
                float l1 = 2 * r_bd_m * L_Q / (p_bl * p_b) * sinc(omega_kg_bd * r_bd_m) * sinc(omega_kq_bd * L_Q / 2);
        ...
        int k_q_bp_min = -k_g_bp_max;
        
        vec2 masking_bp = BarbMasking(r_bl_uv, v_proj, n_bp, phase_bp_0, 0.00005, d_bl_uv);
            ...
            for(int q_bd = k_q_bd_min;q_bd <= k_q_bd_max;q_bd++) {
                for(int g_bp = k_g_bp_min;g_bp <= k_g_bp_max;g_bp++) {
                    for(int q_bp = k_q_bp_min;q_bp <= k_q_bp_max;q_bp++) {
                        ...
                        float l3 = cos(l3_a - dot(omega, mean));
        ...
        vec2 s_boundary_array[16];
        int s_count = 0;

        for (int i = kb_min;i <= kb_max;i++) {
            vec2 masking = BarbMasking(r_b_uv, v_proj, normal_b, i * p_b, 0.00005, p_b - 2 * r_b_uv);
        ...
        vec2 c_kb1 = BarbMasking(r_b_uv, v_proj, normal_b, (kb_max + 1) * p_b, 0.00005, p_b - 2 * r_b_uv);
        s_boundary_array[s_count + 1].x = c_kb1.y;
        s_boundary_array[s_count + 1].y = c_kb1.x;
        s_count += 1;

        vec2 c_kb2 = BarbMasking(r_b_uv, v_proj, normal_b, (kb_min - 1) * p_b, 0.00005, p_b - 2 * r_b_uv);
        ...
        for (int j = kg_min, idx = 0;j <= kg_max;j++, idx++) {
            ...
            float y_barb_bd = j / N_b;
            float phase_base_bd = dot(vec2(0, y_barb_bd), n_bd);

            int k_bd_0 = int((b_bd * mean_z.y + a_bd * mean_z.x - phase_base_bd) / p_bl);
            ...
            float s_r = s_boundary_array[idx + 1].x - s_boundary_array[idx].y;

            vec2 masking_bd[32];
            int bd_masking_count = 0;

            for (int i = k_bd_min;i <= k_bd_max;i++) {
                vec2 masking = BarbMasking(r_bl_uv, v_proj, n_bd, phase_k_bd, 0.0005, d_bl_uv);
            ...
            float y_barb_bp = (j + 1) / N_b;
            float phase_base_bp = dot(vec2(0, y_barb_bp), n_bp);

            int k_bp_0 = int((b_bp * mean_z.y + a_bp * mean_z.x - phase_base_bp) / p_bl);
            ...
            for (int i = k_bd_min;i <= k_bd_max;i++) {
                vec2 masking = BarbMasking(r_bl_uv, v_proj, n_bp, phase_k_bp, 0.0005, d_bl_uv);
            ...
            for (int p = k_bd_min, idx1 = 0;p <= k_bd_max;p++, idx1++) {
    ...
    float w_e = 1 - w_r - w_b - w_bd - w_bp;

    outColor = vec4(w_e * vec3(1.0, 1.0, 1.0) + 
                    w_r * vec3(1.0, 1.0, 0.0) +
                    w_b * vec3(0.0, 1.0, 1.0) +
                    w_bd * vec3(0.0, 1.0, 0.0) +
                    w_bp * vec3(1.0, 0.75, 0.0), 1.0);
}
