...
pub fn build(b: *std.Build) void {
    ...
    if (tracy_callstack) |depth| {
        const depth_str = b.fmt("\"{d}\"", .{depth});
        c.defineCMacro("TRACY_CALLSTACK", depth_str);
    }
    ...
    if (shared and target.result.os.tag == .windows) {
        c.defineCMacro("TRACY_IMPORTS", "1");
    }

    c.addIncludePath(tracy_src.path("public"));

    const c_module = c.createModule();
    tracy_module.addImport("c", c_module);
